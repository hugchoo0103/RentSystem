    ...
    <script>
        ...
        $(function () {
            ...
            $("#button").click(function () {
                if (document.hirePersonform.hireId.value==""||document.hirePersonform.hireId.value==null)
                {
                    window.alert("搜索内容不能为空！");
                    return false;
                }
                else{
                    var newUrl = '${pageContext.request.contextPath}/hirePerson/getHirePersonByNameLimit';    //设置新提交地址
                    var newName = 'userName';
                    var input= $("[name='hireId']").val();
                    ...
                    if (choose == 2) {
                        if (document.hirePersonform.hireId.value.length>10||document.hirePersonform.hireId.value.length<3){
                            window.alert("请输入3-20个字符的房主姓名");
                            return false;
                        }
                        else{
                            $('input[name="hireId"]').attr('name', newName);
                            $("#hirePersonform").attr('action', newUrl);    //通过jquery为action属性赋值
                        }
                    }
                    $("#hirePersonform").submit();    //提交ID为adminform的表单
        ...
        function hasPrePage() {
            if(!${pageinfo.hasPreviousPage}){
                var newUrl = '${pageContext.request.contextPath}/hirePerson/allHirePersonLimit?startIndex=1';    //设置新提交地址
                $("#prepage").attr('href', newUrl);    //通过jquery为href属性赋值
            }
        }
        function hasNextPage() {
            if(!${pageinfo.hasNextPage}){
                var newUrl = '${pageContext.request.contextPath}/hirePerson/allHirePersonLimit?startIndex=${pageinfo.pages}';    //设置新提交地址
...
<nav class="navbar navbar-expand-md navbar-dark bg-dark mb-4">
    ...
    <a class="navbar-brand" href="#">X-Dance Studio</a>
...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-10 column">
            <div class="form-group" style="width: 120px;float: left">
                <select class="form-control choose_id">
                    ...
                    <option id="1" selected>房主ID</option>
            ...
            <form name="hirePersonform" id="hirePersonform" class="form-inline" action="${pageContext.request.contextPath}/hirePerson/getHirePersonById"
                ...
                &nbsp;&nbsp;
                <a  class="btn default-button" href="${pageContext.request.contextPath}/hirePerson/allHirePersonLimit?startIndex=1" style="text-align: center;" type="button"  >
                    撤销</a>
        ...
        <div class="col-md-2 column" style="float: right;">
            <a class="btn add-button" href="${pageContext.request.contextPath}/hirePerson/toAddHirePerson">+ 添加</a>
        </div>
    </div>
    <div class="row clearfix">
        <div class="col-md-12 column">
            <table class="table table-hover table-striped">
                ...
                <%-- 房主从数据库中查询处理，在这个list中便利: foreach --%>
                <tbody>
                <c:forEach var="hirePerson" items="${hirePersonList}">
                    <tr>
                        <td>${hirePerson.hireId}</td>
                        <td>${hirePerson.userName}</td>
                        <td>${hirePerson.sex}</td>
                        <td>${hirePerson.phone}</td>
                        <td>${hirePerson.email}</td>
                        <td>
                            <svg class="bi bi-pencil-square" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456l-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                                <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
                            </svg>
                            <a href="${pageContext.request.contextPath}/hirePerson/toUpdateHirePerson/${hirePerson.hireId}" class="modify-text">修改</a>
                            &nbsp;| &nbsp;
                            ...
                            <a href="${pageContext.request.contextPath}/hirePerson/deleteHirePerson/${hirePerson.hireId}"
    ...
    <div id="pageNav" style="margin-top:25px;visibility: visible;" >
        <nav aria-label="Page navigation">
            <ul class="pagination justify-content-center">
                <form id="pageform" action="${pageContext.request.contextPath}/hirePerson/allHirePersonLimit" method="post">
                    <%-- 前端传递隐藏域 --%>
                    <input type="hidden" id="start_index" name="startIndex">
                    <input type="hidden" id="page_size" name="pageSize">
                </form>
                <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/hirePerson/allHirePersonLimit?startIndex=1" aria-label="Previous">&laquo;</a></li>
                <li class="page-item">
                    <a class="page-link" id="prepage" href="${pageContext.request.contextPath}/hirePerson/allHirePersonLimit?startIndex=${pageinfo.pageNum-1}" aria-label="Previous" onclick="hasPrePage()">
                ...
                <c:forEach var="num"
                    ...
                    <li class="page-item">
                        <a class="page-link" href="${pageContext.request.contextPath}/hirePerson/allHirePersonLimit?startIndex=${num}">
                                <%--判断：主要是为了给当前页显示红色--%>
                            <c:choose>
                ...
                <li class="page-item">
                    <a class="page-link" id="nextpage" href="${pageContext.request.contextPath}/hirePerson/allHirePersonLimit?startIndex=${pageinfo.pageNum+1}" aria-label="Next" onclick="hasNextPage()">
                        <span aria-hidden="true">&gt;</span>
                        <span class="sr-only">Next</span>
                    </a>
                </li>
                <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/hirePerson/allHirePersonLimit?startIndex=${pageinfo.pages}" aria-label="Previous">&raquo;</a></li>
            </ul>
        </nav>
    </div>
