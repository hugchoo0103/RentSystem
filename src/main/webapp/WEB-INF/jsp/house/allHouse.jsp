        ...
        $(function () {
            ...
            $("#button").click(function () {
                if (document.houseform.houseId.value==""||document.houseform.houseId.value==null)
                {
                    ...
                    if (choose == 2) {
                        if (document.houseform.houseId.value=="高级舞蹈室"){
                            document.getElementById("intype").value="S";
                            $('input[name="houseId"]').attr('name', newName);
                            $("#houseform").attr('action', newUrl);    //通过jquery为action属性赋值
                        }
                        else if (document.houseform.houseId.value=="中级舞蹈室"){
                            document.getElementById("intype").value="M";
                            $('input[name="houseId"]').attr('name', newName);
                            $("#houseform").attr('action', newUrl);    //通过jquery为action属性赋值
                        }
                        else if (document.houseform.houseId.value=="个人舞蹈室"){
                            document.getElementById("intype").value="P";
                            $('input[name="houseId"]').attr('name', newName);
                            $("#houseform").attr('action', newUrl);    //通过jquery为action属性赋值
                        }
                        else{
                            window.alert("请输入正确的舞蹈室等级");
                            return false;
                        }
                    }
    ...
    <div style="margin-top:25px;">
        <nav aria-label="Page navigation">
            <ul class="pagination justify-content-center">
                <form name="pageform" id="pageform" action="${pageContext.request.contextPath}/house/allHouseLimit" method="post">
                    <%-- 前端传递隐藏域 --%>
